...
func _on_textbox_show() -> void:
	...
	# Instead of get_parent().get_parent(), we just modulate the BubbleAnchor directly.
	# But actually the animations themselves ("textbox_pop", "textbox_fade_up")
	# already map to "BubbleAnchor:modulate", so we don't need this manual transparent set.
	# We just ensure it's transparent before the animation tracks kick in.
	$"../BubbleAnchor".modulate = Color.TRANSPARENT
...
func _on_textbox_new_text() -> void:
	...
	match animation_new_text:
		AnimationsNewText.WIGGLE:
			# Our scene doesn't have a new_text animation yet, so this gracefully skips
			if has_animation("new_text"):
				play("new_text")
